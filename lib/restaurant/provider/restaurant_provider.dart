...
class RestaurantStateNotifier extends StateNotifier<CursorPaginationBase> {
  ...
  Future<void> paginate({
    ...
    try {
      ...
      // 로딩중인데, fetchMore가 아닐때 - 새로고침의 의도가 있을 수 있다. => 2번 사항에서 제외
      ...
      if (fetchMore) {
        // fetchMore가 실행되는 상황 -> 데이터를 무조건 들고 있을때
        final pState = state as CursorPagination;
      ...
      // 데이터를 처음부터 가져오는 상황
        ...
        // 만약에 데이터가 있는 상황이라면 기존 데이터를 보존한채 Fetch (API 요청)을 진행
        if (state is CursorPagination && !forceRefetch) {
          final pState = state as CursorPaginationRefetching;
  ...
  void getDetail({
    ...
    // 데이터를 가져오는 시도를 한다.
    if (state is! CursorPagination) {
      // 비동기적으로 함수를 실행하므로 paginate 함수는 Future<void>로 반환값을 변경해준다.
      await this.paginate();
    }
